...
SketchPolicy::SketchPolicy(SearchTask task, CostModel program_cost_model,
  ...
  if (IsCPUTask(node->search_task)) {
    ...
    // Mutation Rules for Evolutionary Search
	int limit_blk_num = GetIntParam(node->params, SketchParamKey::limit_blk_num);
	if (limit_blk_num == 1) {
		node->mutation_rules.push_back(std::make_shared<MutateTileSize>(1.0));
	}
	else {
		node->mutation_rules.push_back(std::make_shared<MutateTileSize>(0.90));
...
State SketchPolicyNode::Search_without_measure(int n_trials, int early_stopping, int num_measure_per_iter,
	ProgramMeasurer measurer) {
	num_measure_per_iter_ = num_measure_per_iter;
	...
	if (n_trials <= 1) {
		...
		while (ct < n_trials) {
			...
			// Measure candidate states--> we do not measure the candidate states here, use the predicted value only
			std::vector<float> pred_scores;
			Array<State> cand_states;
			for (auto tmp_inp : inputs) {
				cand_states.push_back(tmp_inp->state);
			}
			pred_scores.reserve(cand_states.size());
			program_cost_model->Predict(search_task, cand_states, &pred_scores);

			PrintTitle("Measure", verbose);
			//results = measurer->Measure(search_task, GetRef<SearchPolicy>(this), inputs);
			measurer->FakeMeasure(search_task, GetRef<SearchPolicy>(this), inputs, pred_scores);
			ct += inputs.size();
			...
			for (float pred_scr : pred_scores) {
				float flops = pred_scr;
				if (pred_scr <= 0) {
					flops = 0;
				}
				measured_states_throughputs_.push_back(flops);
			}
		}
		PrintTitle("Done", verbose);
...
Array<MeasureInput> SketchPolicyNode::ContinueSearchOneRound_without_measure(
	int num_measure, ProgramMeasurer measurer) {
	num_measure_per_iter_ = num_measure;
	...
	// Measure candidate states (we do not measure states here)
	std::vector<float> pred_scores;
	Array<State> cand_states;
	for (auto tmp_inp : inputs) {
		cand_states.push_back(tmp_inp->state);
	...
	// Update the cost model
	PrintTitle("Do not need to train cost model", verbose);
	//program_cost_model->Update(inputs, results);
...
Array<State> SketchPolicyNode::GetMeasuredStates() {
	Array<State> ret(measured_states_vector_.begin(), measured_states_vector_.end());
	return ret;
}

//Array<float> SketchPolicyNode::GetMeasuredThroughputs() {
//	Array<float> ret(measured_states_throughputs_.begin(), measured_states_throughputs_.end());
//	return ret;
//}
...
TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicySearch")
.set_body_typed([](SketchPolicy policy, int n_trials, int early_stopping, int num_measure_per_iter, ProgramMeasurer measurer) {
	State best_state;
	best_state = policy->Search(n_trials, early_stopping, num_measure_per_iter, measurer);
	return best_state;
});


TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicySearchWithoutMeasure")
.set_body_typed([](SketchPolicy policy, int n_trials, int early_stopping, int num_measure_per_iter, ProgramMeasurer measurer) {
	State best_state;
	best_state = policy->Search_without_measure(n_trials, early_stopping, num_measure_per_iter, measurer);
	return best_state;
});


TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicyContinueSearchOneRoundWithoutMeasure")
.set_body_typed([](SketchPolicy policy, int num_measure, ProgramMeasurer measurer) {
	Array<MeasureInput> inputs;
	inputs = policy->ContinueSearchOneRound_without_measure(num_measure, measurer);
	return inputs;
});

TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicyGetMeasuredStates")
.set_body_typed([](SketchPolicy policy) {
	return policy->GetMeasuredStates();
});


//TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicyGetMeasuredThroughputs")
//.set_body_typed([](SketchPolicy policy) {
//	return policy->GetMeasuredThroughputs();
//});

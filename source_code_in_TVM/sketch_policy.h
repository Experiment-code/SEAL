...
struct SketchParamKey {
  ...
  /*! \brief The specified block number. */
  static constexpr const char* limit_blk_num = "limit_blk_num";
  /*! \brief The specified cooperative fetch number. */
  static constexpr const char* limit_fetch_num = "limit_fetch_num";
  /*! \brief The specified micro-kernel shape. */
  static constexpr const char* mick_shape = "mick_shape";
};
...
class SketchPolicyNode : public SearchPolicyNode {
 public:
  ...
  State Search_without_measure(int num_measure_trials, int early_stopping, int num_measures_per_round,
  ...
  Array<MeasureInput> ContinueSearchOneRound_without_measure(
	  int num_measure, ProgramMeasurer measurer);

  Array<State> GetMeasuredStates();

  //Array<float> GetMeasuredThroughputs();

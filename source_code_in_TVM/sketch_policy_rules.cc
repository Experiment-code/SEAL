...
PopulationGenerationRule::ResultKind InitFillTileSize::Apply(SketchPolicyNode* policy, State* state,
	...
	//StdCout(2) << "init fill tile size 0" << std::endl;
  SplitFactorizationMemo split_memo;
  int max_innermost_split_factor =
      GetIntParam(policy->params, SketchParamKey::max_innermost_split_factor);

  /*add to control the number of thread blocks and the number of cooperative fetches in the schedule. BEGIN*/
  int limit_blk_num = 
	  GetIntParam(policy->params, SketchParamKey::limit_blk_num);

  int limit_fetch_num =
	  GetIntParam(policy->params, SketchParamKey::limit_fetch_num);

  //StdCout(2) << "init fill tile size 1" << std::endl;
  std::string mick_shape = GetStringParam(policy->params, SketchParamKey::mick_shape);
  //StdCout(2) << "init fill tile size 2" << mick_shape << std::endl;
  std::vector<int> mick_extents;
  for (size_t iter_i = 0; iter_i < mick_shape.size(); iter_i = iter_i + 5) {
	  mick_extents.push_back(std::stoi(mick_shape.substr(iter_i, 5)));
  }

  int mick_extent_i = 0;

  //StdCout(2) << "mick_shape:" << mick_shape << "number of mick_extents:" << mick_shape.size() << "mick_extents:" << mick_extents[0] << mick_extents[1]<< mick_extents[2] << std::endl;
  /*END---------------------------------------------------------------------------------------------------*/
  ...
  for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
    if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
	  ...
	  /*deal with the limit_blk_num and limit_fetch_num. BEGIN*/
	  /*const Stage& stage = (*state)->stages[ps->stage_id];
	  StdCout(2) << ":00:" << (*state)->stages.size() << stage->iters.size()<< ps->stage_id<< ps->iter_id << std::endl;
	  auto iter = stage->iters[ps->iter_id];
	  StdCout(2) << "1." << limit_blk_num << limit_fetch_num << std::endl; 
	  StdCout(2) << (iter->iter_kind == IteratorKind::kSpatial) << (iter->iter_kind == IteratorKind::kReduction) << std::endl;
	  if (((iter->iter_kind == IteratorKind::kSpatial) && (limit_blk_num == 1)) || 
		  ((iter->iter_kind == IteratorKind::kReduction) && (limit_fetch_num == 1))) {*/
	  if (((ps->lengths.size()==4) && (limit_blk_num == 1)) ||
		  ((ps->lengths.size()==2) && (limit_fetch_num == 1))) {
		  //limit the outermost tile size to be 1
		  //StdCout(2) << "2." << std::endl;
		  extent = mick_extents[mick_extent_i++];
		  //StdCout(2) << "iter" << mick_extent_i-1 << "extent" << extent << "ps->lengths.size()-1 = " << ps->lengths.size() - 1 << std::endl;
		  const auto& candidate_lens = split_memo.GetFactorizationSchemes(extent, ps->lengths.size()-1,
			  max_innermost_split_factor);
		  
		  ICHECK(!candidate_lens.empty());
		  //StdCout(2) << "3." << std::endl;
		  const auto& candidate_lengths = candidate_lens[(*rand_gen)() % candidate_lens.size()];
		  //StdCout(2) << "4." << std::endl;
		  std::vector<int> lengths(ps->lengths.size(), 1);
		  for (int i = 0; i < static_cast<int>(candidate_lengths.size()); ++i) {
			  lengths[i + 1] = candidate_lengths[i]->value;
		  }
		  //StdCout(2) << "5." << std::endl;
		  lengths[0] = extent / ElementProduct(lengths);
		  //StdCout(2) << "6." << std::endl;
		  Array<Integer> fill_cand_lengths;
		  for (size_t j = 0; j < lengths.size(); ++j) {
			  fill_cand_lengths.push_back(Integer(lengths[j]));
		  }
		  //StdCout(2) << fill_cand_lengths  << std::endl;
		  pstate->transform_steps.Set(
			  step_id,
			  SplitStep(ps->stage_id, ps->iter_id, ps->extent,
				  Array<Optional<Integer>>(fill_cand_lengths.begin(), fill_cand_lengths.end()),
				  ps->inner_to_outer));
	  }
	  else {
		  //StdCout(2) << "8." << std::endl;
...
PopulationGenerationRule::ResultKind InitUnroll::Apply(SketchPolicyNode* policy, State* state,
                                                       std::mt19937* rand_gen) const {
	//StdCout(2) << "init unroll 0" << std::endl;
  ...
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
	...
	//StdCout(2) << "init unroll 0.5" << std::endl;
    if (HasReduceIter(stage)) {
      // Use auto unroll for multi level tiled stage
	  // simplify the unrolling factor as DietCode does
	  // <borrowed from dietcode>
		int value;
		int limit_blk_num = GetIntParam(policy->params, SketchParamKey::limit_blk_num);
		//StdCout(2) << "init unroll 1"<<std::endl;
		if (IsGPUTask(policy->search_task) && (limit_blk_num == 1)) {
			size_t unrolling_factor = 1;
			//StdCout(2) << "init unroll 2" << std::endl;
			for (size_t i = 0; i < (*state)->transform_steps.size(); ++i) {
				if (const SplitStepNode* const split_step =
					...
					//StdCout(2) << "init unroll 3 iter" << i << std::endl;
					if (split_step->lengths.size() == 4) {
						//CHECK(split_step->lengths[2].value()->value == 1);
						unrolling_factor *= (split_step->lengths[0].value()->value
							* split_step->lengths[2].value()->value
							* split_step->lengths[3].value()->value);
					}
					else if (split_step->lengths.size() == 2) {
						//we only deal with multi-level tiles
						//CHECK(split_step->lengths.size() == 2);
						//CHECK(split_step->lengths[0].value()->value == 1);
						unrolling_factor *= (split_step->lengths[1].value()->value
							* split_step->lengths[0].value()->value);
					}
				}
			}  // for (i ¡Ê (*state)->transform_steps)

			value = unrolling_factor;
			if (value > 2500) {
				return ResultKind::kInvalid;
			}
		}
		...
		//StdCout(2) << "init unroll 4 unrolling factor" << value << std::endl;
...
PopulationGenerationRule::ResultKind InitThreadBind::Apply(SketchPolicyNode* policy, State* state,
                                                           std::mt19937* rand_gen) const {
	//StdCout(2) << "init thread binding 0" << std::endl;
...
static PopulationGenerationRule::ResultKind AdjustUnrollingFactor(State* state) {
	...
	new_stage.CopyOnWrite()->attrs.auto_unroll_max_step = unrolling_factor;
...
PopulationGenerationRule::ResultKind MutateTileSize::Apply(SketchPolicyNode* policy, State* state,
  ...
  StdCout(2) << "mick_shape:" << mick_shape << "mick_extents:" << mick_extents[0] << mick_extents[1] << mick_extents[2] << std::endl;*/
  /*END---------------------------------------------------------------------------------------------------*/
  ...
  /*const Stage& stage = (*state)->stages[ps->stage_id];
  auto iter = stage->iters[ps->iter_id];*/
  std::vector<int> tmp_lengths(ps->lengths.size() + 1, 1);
  std::vector<int> lengths;
  for (int i = 0; i < static_cast<int>(ps->lengths.size()); ++i) {
	  tmp_lengths[i + 1] = GetIntImm(ps->lengths[i].value());
  }
  tmp_lengths[0] = extent / ElementProduct(tmp_lengths);
  /*bool not_consider_first_length =
	  ((!(StrEndsWith(stage->op->name, ".shared"))) &&
	  (((iter->iter_kind == IteratorKind::kSpatial) && (limit_blk_num == 1)) ||
	  ((iter->iter_kind == IteratorKind::kReduction) && (limit_fetch_num == 1))));*/
  bool not_consider_first_length =
	  (((ps->lengths.size() > 2) && (limit_blk_num == 1)) ||
	  ((ps->lengths.size() == 2) && (limit_fetch_num == 1)));

  if (not_consider_first_length) {
	  for (size_t i = 1; i < tmp_lengths.size(); ++i) {
		  lengths.push_back(tmp_lengths[i]);
	  }
  }
  else {
	  for (size_t i = 0; i < tmp_lengths.size(); ++i) {
		  lengths.push_back(tmp_lengths[i]);
	  }
  }
  /*END------------------------------------------------------*/
  ...
  for (size_t i = 0; i < random_perm.size(); ++i) {
	...
	//<borrow from dietcode> adjust the unrolling factor
    //return ResultKind::kValid;
	if (limit_blk_num == 1) {
		return AdjustUnrollingFactor(state);
	}
	else {
		return ResultKind::kValid;
	}

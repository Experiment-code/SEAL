...
void ProgramMeasurerNode::FakeMeasure(const SearchTask& task,
	const SearchPolicy& policy,
	const Array<MeasureInput>& inputs,
	//int batch_size,
	...
	StdCout(verbose) << "Get " << inputs.size() << " programs to measure (not measured):" << std::endl;

	for (size_t i = 0; i < inputs.size(); i += 1) {
		const String& workload_key = inputs[i]->task->workload_key;
		double flops;
		double result;
		if (pred_scores[i] > 0) {
			flops = pred_scores[i];
		...
		if (flops > best_flops[workload_key]) {
			best_flops[workload_key] = flops;
			best_state[workload_key] = inputs[i]->state;
			best_ct[workload_key] = ct;
		}
		...
		StdCout(verbose, 2) << std::fixed << std::setprecision(2) << Chars('=', 50) << "\n"
			<< "No: " << ct << "\tGFLOPS: " << flops / 1e9 << " / "
			<< best_flops[workload_key] / 1e9 << "\tresults: " << result
			<< "\n"
			<< Chars('=', 50) << "\n"
			<< inputs[i]->state << "\n";

...
class SketchPolicy(SearchPolicy):
    ...
    def search(
        self, n_trials, early_stopping, 
        num_measure_per_iter, measurer):
        """Seach the best state for the task, the original search method
        of Ansor.
        The actual search is all done in c++.

        Parameters
        ----------
        The parameters are exactly the same as in the c++ method.

        Returns
        -------
        best_state : State
            The best state found during search.
        """
        best_state = _ffi_api.SketchPolicySearch(
            self, n_trials, early_stopping, 
            num_measure_per_iter, measurer)
        return best_state


    def search_without_measure(
        self, n_trials, early_stopping, 
        num_measure_per_iter, measurer):
        """Seach the best state for the task,
        fully depending on the cost model.
        The actual search is all done in c++.
        ...
        best_state : State
            The best state found during search.
        """
        best_state = _ffi_api.SketchPolicySearchWithoutMeasure(
            self, n_trials, early_stopping, 
            num_measure_per_iter, measurer)
        return best_state


    def continue_search_one_round_without_measure(
        self, num_measure, measurer):
        """Seach the best state for the task for one continued round,
        fully depending on the cost model.
        The actual search is all done in c++.
        ...
        best_state : State
            The best state found during search.
        """
        inputs = _ffi_api.SketchPolicyContinueSearchOneRoundWithoutMeasure(
            self, num_measure, measurer)
        return inputs


    def get_measured_states(self):
        """Return all the measured states which are stored in policy.
        The actual search is all done in c++.
        ...
        states : List[State]
           The measured states.
        """
        states = _ffi_api.SketchPolicyGetMeasuredStates(self)
        return states
